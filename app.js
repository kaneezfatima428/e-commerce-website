...
//         if (typeof animateProductCards === "function") {
//             animateProductCards();
//         }
//     }
// }
...
function displayProducts(filteredArray) {
    ...
    // Dynamic GSAP Animation Trigger Jab Products Render Hote Hain
    if (typeof animateProductCards === "function") {
        animateProductCards();
    }
}
...
function addProductToCart(productId) {
    ...
    if (selectedItem) {
        ...
        // Navbar counter badge update
        cartCountBadge.innerText = myCart.length;
        cartCountBadge.style.display = "inline-block";

        // Cart Badge Pulse Animation (GSAP)
...
// 6. Dynamic Modal Loader Generation Engine
function updateCartModalUI() {
    cartModalBody.innerHTML = "";
    var totalBillAmount = 0;
...
// 8. Checkout Se Pehle Cart Validation Engine
function checkCartBeforeCheckout() {
    if (myCart.length === 0) {
        var cartModalEl = document.getElementById('cartModal');
        var cartModalInstance = bootstrap.Modal.getInstance(cartModalEl);
